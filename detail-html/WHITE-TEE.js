...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/WHITE-TEE/WHITE-TEE-front.png', // Update with correct image path
        type: 'WHITE TEE'
    };
    ...
    showToast(`Added ${quantity} WHITE TEE${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.whitetee && data.products.whitetee.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.whitetee.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.whitetee.stock_status;
        } else {
