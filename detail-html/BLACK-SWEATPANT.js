...
function generateImagePaths() {
    const baseFolder = './BLACK-SWEATPANT/';
    const images = [];
    
    // Add all images from the folder
    const imageNames = [
        'BLACK-SWEATPANT-front.png',
        'BLACK-SWEATPANT-back.png'
    ];
...
function addToCart() {
    ...
    const price = 35.00; // Set your hoodie price
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/BLACK-SWEATPANT/BLACK-SWEATPANT-front.png', // Update with correct image path
        type: 'BLACK SWEATPANT'
    };
    ...
    showToast(`Added ${quantity} BLACK SWEATPANT${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.blacksweatpant && data.products.blacksweatpant.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.blacksweatpant.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.blacksweatpant.stock_status;
        } else {
