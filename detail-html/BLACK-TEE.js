...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/BLACK-TEE/BLACK-TEE-front.png', // Update with correct image path
        type: 'BLACK TEE'
    };
    ...
    showToast(`Added ${quantity} BLACK TEE${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.blacktee && data.products.blacktee.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.blacktee.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.blacktee.stock_status;
        } else {
