...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/BLACK-SWEATER/BLACK-SWEATER-front.png', // Update with correct image path
        type: 'BLACK SWEATER'
    };
    ...
    showToast(`Added ${quantity} BLACK SWEATER${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.blacksweater && data.products.blacksweater.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.blacksweater.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.blacksweater.stock_status;
        } else {
