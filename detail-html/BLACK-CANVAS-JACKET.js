...
function generateImagePaths() {
    const baseFolder = './BLACK-CANVAS-JACKET/';
    const images = [];
    
    // Add all images from the folder
    const imageNames = [
        'BLACK-CANVAS-JACKET-front.png',
        'BLACK-CANVAS-JACKET-back.png'
    ];
...
function addToCart() {
    ...
    const price = 40.00; // Set your hoodie price
    ...
    const item = {
        ...
        name: 'BLACK CANVAS JACKET',
        size: selectedSize,
        quantity: quantity,
        price: price,
        image: './detail-image/BLACK-CANVAS-JACKET/BLACK-CANVAS-JACKET-front.png', // Update with correct image path
        type: 'BLACK CANVAS JACKET'
    };
    ...
    showToast(`Added ${quantity} BLACK CANVAS JACKET${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.blackcanvasjacket && data.products.blackcanvasjacket.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.blackcanvasjacket.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.blackcanvasjacket.stock_status;
        } else {
