...
function generateImagePaths() {
    const baseFolder = './BABY-BLUE-TEE/';
    const images = [];
    
    // Add all images from the folder
    const imageNames = [
        'BABY-BLUE-TEE-front.png',
        'BABY-BLUE-TEE-back.png'
    ];
...
function addToCart() {
    ...
    const price = 25.00; // Set your hoodie price
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/BABY-BLUE-TEE/BABY-BLUE-TEE-front.png', // Update with correct image path
        type: 'BABY BLUE TEE'
    };
    ...
    showToast(`Added ${quantity} BABY BLUE TEE${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.babybluettee && data.products.babybluettee.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.babybluettee.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.babybluettee.stock_status;
        } else {
